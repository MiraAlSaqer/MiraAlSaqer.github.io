...
document.addEventListener("DOMContentLoaded", function () {
  // Find all your game preview video embeds using the class we added
  const projectIframes = document.querySelectorAll(".project-iframe");

  // Trigger the video the second 20% of the card pops up on screen
  ...
  const videoObserver = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
      // The exact moment the card appears during a scroll
      if (entry.isIntersecting) {
        const iframe = entry.target;
        let currentSrc = iframe.getAttribute("src");
        
        // Dynamic replacement: swap the static URL for an autoplaying version
        if (!currentSrc.includes("autoplay=1")) {
          const separator = currentSrc.includes("?") ? "&" : "?";
          iframe.setAttribute("src", currentSrc + separator + "autoplay=1");
        }
        
        // Stop tracking this specific card so the video can just loop smoothly
        videoObserver.unobserve(iframe);
      }
    });
  }, observerOptions);

  // Arm the scroll observer on all six cards
